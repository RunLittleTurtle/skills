...
Mise à jour : `/plugin marketplace update skills`. Désinstallation : `/plugin uninstall skill-creator@skills`.
...
cp -R skills/plugins/skill-creator/skills/skill-creator <DOSSIER_SKILLS_DE_TON_OUTIL>/
```
...
/skill-creator:skill-creator /path/to/reference.md
/skill-creator:skill-creator /path/to/file1.py /path/to/file2.py
```
...
- **Aucun chemin/nom hardcodé** : tout passe par les variables de ta config (`~/.claude/skill-creator/config.json`).
...
- `skills/skill-creator/SKILL.md` — workflow principal (détection référence, branchement A/B/C, étapes 0 à 11).
- `skills/skill-creator/references/AUTHORING_GUIDELINES.md` — limites officielles, exemples bons/mauvais, anti-patterns (lu pendant la rédaction du body).
- `skills/skill-creator/references/REFERENCE_DETECTION.md` — workflow détaillé de l'Étape -1 (détection paths → analyse réelle des fichiers → liste DYNAMIQUE de composantes).
- `skills/skill-creator/assets/readme-template.md` — template du README de marketplace (utilisé au bootstrap d'un nouveau repo).
- `skills/skill-creator/assets/readme-plugin-template.md` — template du README généré à la racine de chaque nouveau plugin.
