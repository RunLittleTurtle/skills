---
name: skill-creator-generic
description: Create a new agent skill OR modify an existing one, in a personal marketplace (Type A, stable or beta), as a personal standalone (Type B), or for another tool compatible with agentskills.io (Type C, e.g. Codex, Antigravity, OpenCode, Cursor, Copilot, Claude Desktop). Identity-free and runtime-configured. Separates the GitHub repo name (e.g. `skills`) from the marketplace identifier (e.g. `<github_user>-skills`) so that two users with the same repo name never collide inside Claude Code. Auto-detects reference files passed as arguments and skills installed on the machine. Preserves the original slug on modification, snapshots the skill into /tmp/ before editing. Persists config in ~/.claude/skill-creator-generic/config.json. Invoke when the user wants to create or modify a skill, regardless of setup or intent.
---

# skill-creator-generic — Create or modify a skill (collision-safe fork)
...
This fork of [`flo351/skill-creator`](https://github.com/flo351/skills) adds one structural fix: the **marketplace identifier** (the `name:` field inside `.claude-plugin/marketplace.json`, used by Claude Code to register the marketplace) is now a distinct config field from the **GitHub repo name** (the slug at `https://github.com/<user>/<repo>`). Defaulting the marketplace identifier to `<github_user>-<repo_name>` guarantees that two users whose repos happen to share a slug (very common — many people name theirs `skills`) won't overwrite each other's marketplace registration on `/plugin marketplace add`.
...
Config lives in `~/.claude/skill-creator-generic/config.json` and supports two contexts (stable and beta). The schema separates the GitHub repo name from the Claude Code marketplace identifier — see "Why two names" in the Design rationale.
...
```json
{
  ...
  "repo_name": "<stable GitHub repo slug, e.g. skills>",
  "repo_marketplace_name": "<stable marketplace identifier, e.g. acme-skills>",
  "repo_dir": "<absolute path to stable repo>",
  "repo_url": "https://github.com/<login>/<stable repo name>.git",
  "repo_visibility": "public" | "private",

  "beta_repo_name": "<beta GitHub repo slug, e.g. skills-beta>",
  "beta_repo_marketplace_name": "<beta marketplace identifier, e.g. acme-skills-beta>",
...
>
> - **Beta** (`<beta_repo_marketplace_name>`, recommended for a new creation — you can promote to stable later): `<beta_repo_dir>`
> - **Stable** (`<repo_marketplace_name>`, direct public sharing): `<repo_dir>`"

Store the choice in a local variable `<context>` (values `"beta"` or `"stable"`). The rest of Step 0 uses the matching variables (`repo_name`/`beta_repo_name`, `repo_marketplace_name`/`beta_repo_marketplace_name`, etc.) based on this choice.

### 0.2 — If config exists and is valid

Load `~/.claude/skill-creator-generic/config.json`, verify required keys for the chosen context are present (including the new `<context>_marketplace_name`). If a config from an earlier version is missing `<context>_marketplace_name`, prompt for it (Step 0.3) before continuing.

Show a summary: "Mode: <github|local>, repo <context>: `<repo_name>` (GitHub) → marketplace id `<repo_marketplace_name>`, dir: `<repo_dir>`, visibility: <visibility>", then move to Step 1.
...
|---|---|---|
| `<context>_name` (GitHub repo slug) | suggest `"skills"` / `"skills-beta"` | same |
| `<context>_marketplace_name` (Claude Code id) | suggest `"<github_user>-<context>_name"` (e.g. `runlittleturtle-skills`) | suggest `"<author_name kebab-case>-<context>_name"` |
...
When asking for `<context>_marketplace_name`, explain the why in one short sentence: *"This name is the identifier Claude Code uses to register your marketplace and route `/plugin install foo@<name>`. It must be unique across every marketplace any user has ever installed. Prefixing with your GitHub login avoids collisions with people who picked the same repo name."*

### 0.4 — Save

Write config to `~/.claude/skill-creator-generic/config.json` (mkdir first). Confirm: "Config saved. You can edit it any time in that file."
...
- If `<dir>` exists with `.claude-plugin/marketplace.json` → use as-is, but verify the top-level `name` matches `<context>_marketplace_name` from config. If not, warn and offer to align.
...
1. Confirm via `AskUserQuestion`: "Create repo `<github_user>/<context>_name` now? It will register inside Claude Code under the marketplace identifier `<context>_marketplace_name`."
...
**`marketplace.json` (top-level identifier)**: the `name` field at the root MUST equal `<context>_marketplace_name` (not `<context>_name`). This is the collision-safe identifier. Default skeleton:

```json
{
  "name": "<context>_marketplace_name",
  "description": "<beta: 'BETA — unstable versions, personal experiments' / stable: neutral one-liner>",
  "owner": {
    "name": "<author_name>",
    "url": "<author_url>"
  },
...
**`README.md`**: read `${CLAUDE_SKILL_DIR}/assets/readme-template.md`, substitute placeholders (note: the template uses `<marketplace_name>` for commands after `marketplace add`, not `<repo_name>`). For the beta context, prepend the warning banner "Personal use, unstable versions".
...
- `<slug>`, `<description>`, `<github_user>`, `<context>_name` (for the `marketplace add` URL), `<context>_marketplace_name` (for `/plugin install foo@…` and update/uninstall commands) → as before.
...
Read `<dir>/.claude-plugin/marketplace.json`. Verify its top-level `name` equals `<context>_marketplace_name` from config — if not, warn the user (likely a marketplace created before the collision fix) and offer to fix it now. Then append an entry to the `plugins` array:
...
Marketplace id   : <context>_marketplace_name  (used by Claude Code to register and route commands)
Install          : /plugin marketplace add <github_user>/<context>_name
                   /plugin marketplace update <context>_marketplace_name
                   /plugin install <slug>@<context>_marketplace_name
Slash            : /<slug>:<slug>
```

**Type A, not pushed**: add `To do: cd <dir> && git add -A && git commit -m "..." && git push`.

**Type A, local mode**: replace the GitHub URL with `Install: /plugin marketplace add <dir>` and mention sharing via zip + `marketplace add <path>` on the recipient's side. The marketplace id `<context>_marketplace_name` is still what subsequent commands use.
...
### Why two names (`<context>_name` vs `<context>_marketplace_name`)
GitHub identifies a repo as `<owner>/<repo>`, so two users named `alice` and `bob` can both own a repo called `skills` without conflict — the owner differentiates the URL. Claude Code, on the other hand, identifies a marketplace by the top-level `name` field inside `.claude-plugin/marketplace.json` alone. If `alice/skills` and `bob/skills` both ship `"name": "skills"`, then running `/plugin marketplace add bob/skills` after `alice/skills` overwrites Alice's registration silently. The fix is to keep the GitHub repo slug short and natural (`skills`, `skills-beta`) and prefix the marketplace identifier with the user's GitHub login (e.g. `alice-skills`, `bob-skills`). Two separate fields in the config make this explicit.
...
This skill is a fork of [`flo351/skills` → `skill-creator`](https://github.com/flo351/skills/tree/main/plugins/skill-creator) by [@flo351](https://github.com/flo351), which is itself derived from `skill-creator-turtle` by [@RunLittleTurtle](https://github.com/RunLittleTurtle). The only structural change in this fork is the introduction of `<context>_marketplace_name` as a config field distinct from `<context>_name`, so that marketplace identifiers are guaranteed unique between users even when GitHub repo slugs collide. MIT licensed.
