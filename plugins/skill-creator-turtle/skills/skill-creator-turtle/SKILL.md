...
# skill-creator-turtle — Créer ou modifier un skill (version beta alignée Anthropic)
...
Le skill `skill-creator-turtle` détecte son propre contexte via son `name:` (suffixe `-beta` → contexte beta par défaut). Mais l'utilisateur peut vouloir créer un skill stable depuis l'invocation beta (par exemple ajouter directement à la marketplace stable sans passer par beta). Demande via `AskUserQuestion` :
