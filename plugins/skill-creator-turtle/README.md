...
Tu veux créer un skill Claude Code et tu ne veux pas réinventer la structure (`plugin.json` + `marketplace.json` + `SKILL.md` + README + commit + push) à la main. Le skill-creator-turtle gère **3 cibles** :
...
Mise à jour : `/plugin marketplace update skills`. Désinstallation : `/plugin uninstall skill-creator-turtle@skills`.
...
cp -R skills/plugins/skill-creator-turtle/skills/skill-creator-turtle <DOSSIER_SKILLS_DE_TON_OUTIL>/
```
...
/skill-creator-turtle:skill-creator-turtle

# Création avec analyse de fichier(s) de référence
/skill-creator-turtle:skill-creator-turtle /path/to/reference.md
/skill-creator-turtle:skill-creator-turtle /path/to/file1.py /path/to/file2.py
```
...
- `skills/skill-creator-turtle/SKILL.md` — workflow principal (détection référence, branchement A/B/C, étapes 0 à 11).
- `skills/skill-creator-turtle/references/AUTHORING_GUIDELINES.md` — limites officielles, exemples bons/mauvais, anti-patterns (lu pendant la rédaction du body).
- `skills/skill-creator-turtle/references/REFERENCE_DETECTION.md` — workflow détaillé de l'Étape -1 (détection paths → analyse réelle des fichiers → liste DYNAMIQUE de composantes).
- `skills/skill-creator-turtle/assets/readme-template.md` — template du README de marketplace (utilisé au bootstrap d'un nouveau repo).
- `skills/skill-creator-turtle/assets/readme-plugin-template.md` — template du README généré à la racine de chaque nouveau plugin.
