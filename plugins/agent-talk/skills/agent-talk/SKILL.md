...
# agent-talk — communication inter-instances Claude
...
Pas de blocage ici — `agent-talk` fonctionne aussi hors repo et sans coordination (cross-projet via path absolu désigné).
...
dir="$CWD"
while [ "$dir" != "/" ] && [ "$dir" != "$HOME" ] && [ -n "$dir" ]; do
  for f in "$dir"/.agent-talk/*/BRIDGE.README.md; do
    [ -e "$f" ] || continue
    echo "$f"
  done
  dir=$(dirname "$dir")
...
Cela couvre :
- `<CWD>/.agent-talk/*/` (je suis l'agent host)
...
```bash
find "$CWD" -maxdepth 5 -type f -name BRIDGE.README.md -path '*/.agent-talk/*' 2>/dev/null
```

(maxdepth 5 = `<CWD>/<a>/<b>/<c>/.agent-talk/<topic>/BRIDGE.README.md` — suffisant pour la plupart des cas)
...
> *"Aucun bridge agent-talk détecté. Que veux-tu faire ?"*
...
**Si same-repo nested** :
- Path par défaut = `<CWD>/.agent-talk/<topic>/`
- Confirme à l'utilisateur en une phrase : *"Bridge créé dans `<CWD>/.agent-talk/<topic>/`. Si ton autre instance Claude a un cwd PLUS PROFOND, elle accédera quand même au bridge (mais idéalement le bridge devrait être chez elle — tu peux migrer manuellement plus tard). Si elle a un cwd au-dessus, elle y accède aussi."*
...
- Si `NOTIF_FILE = <COORD_FOCUS_FILE>` (same-repo + COORDINATION) : append au log :
  ...
  ```
- Si mode same-repo : propose à l'utilisateur (en une ligne, pas `AskUserQuestion`) : *"Ajouter `.agent-talk/` au `.gitignore` du repo pour ne pas committer le bridge ? (Réponds dans le chat si oui.)"* — ne le fais PAS automatiquement.
...
- **Cas warning** : si `BRIDGE_MODE = same-repo` ET `CWD` est plus profond que `host_cwd` (string strictly longer + startswith host_cwd), signale à l'utilisateur : *"Ton cwd `<CWD>` est plus profond que le host `<host_cwd>`. Idéalement le bridge devrait être chez toi. Tu peux migrer manuellement (`mv <bridge-root> <CWD>/.agent-talk/<topic>/`) puis update BRIDGE.README.md, mais c'est optionnel — le bridge fonctionne quand même."* Ne fais rien automatiquement.
...
>
> **Pas de locks** : agent-talk n'utilise pas le système `[LOCKED]` de coordination. Le turn marker `**Tour : @inst-X**` est le protocole d'exclusion. Si tu suspectes une collision réelle (deux agents écrivent en même temps), demande à l'humain d'arbitrer.
>
...
> Protocole et légende du bridge agent-talk.
> Pour rejoindre : lance `/agent-talk` et choisis "Connecter à `<topic>`".
...
- En mode same-repo, le bridge est dans `.agent-talk/<topic>/` — pense à l'ajouter au `.gitignore` du repo.
...
- **Pas de modification du structure de COORDINATION.FOCUS.md** : agent-talk n'ajoute que des entrées (`[BRIDGE-CREATED]`, `[PING]`, `[READ-PING]`, `[CLOSED]`) au log activité, jamais ne change le format ou les tableaux existants.
